...
module.exports = {
  ...
  decodeUsername: (messageContent) => {
    const splitUp = messageContent.split(" ");
    splitUp.shift();
    const lastWord = splitUp[splitUp.length - 1].split("#");
    const discriminator = lastWord.pop();
    splitUp.pop();
    const all_usernames = splitUp.concat(lastWord);
    const username = all_usernames.join(" ");
